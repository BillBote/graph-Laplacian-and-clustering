...
def _fit_owl_fista(X, y, w, loss, max_iter=10000, max_linesearch=20, eta=2.0,
                   tol=1e-3, verbose=0):
    ...
    return pfista(sfunc, nsfunc, coef, max_iter, max_linesearch,
                 eta, tol, verbose)
...
class _BaseOwl(BaseEstimator):
    ...
    def __init__(self, weights, loss='squared', max_iter=10000,
...
def _owl_ramp_weights(N, r, l, t):
    """define OWL_Ramp weights

    parameter
    ---------
    N: number of data points
    r: a non-negative integer no bigger than N
    l: l1 parameter
    t: gap size

    """
    if N<r:
        print("r should be not larger than N")
    else:
        w=np.ones(N)*l
        for i in range(r):
            w[i]=w[i]+(r-i+1)*t
